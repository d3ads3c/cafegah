...
export default function MySubs() {
    ...
    const GetInvitations = async () => {
        try {
            ...
            } else if (data && data.Status === "Success" && Array.isArray(data.Invitations)) {
            ...
            const filteredInvitations = invitationsList.filter(inv => inv.Status === 'invited');
    ...
    return (
        <div className="my-5">
            {invitations && invitations.length > 0 && (
                <>
                    <div className="">
                        <h2 className="text-lg font-bold text-gray-900">دعوت نامه های انتظار</h2>
                    </div>
                    <div className="mt-4 flex w-full overflow-auto gap-5">
                        {invitations && invitations.length > 0 ? (
                            invitations.map((inv) => (
                                <div key={inv.InvitationID} className="w-[450px] min-w-[450px] rounded-2xl bg-white border border-gray-100 p-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="w-full">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    <h2 className="font-bold">{inv.CafeName || 'بدون نام'}</h2>
                                                    <p className="text-gray-400 text-xs">{inv.Plan || '-'}</p>
                                                </div>
                                    ...
                                    <div className="my-4 p-3 bg-blue-50 rounded-lg">
                                        <p className="text-xs text-gray-600 mb-1"><span className="font-bold">مالک:</span> {inv.Owner}</p>
                                        <p className="text-xs text-gray-600"><span className="font-bold">دسترسی‌ها:</span> {inv.Permissions.split(',').length} دسترسی</p>
                                    </div>
                            ...
                            <div className="text-gray-400 text-sm">دعوت نامه‌ای در انتظار ندارید</div>
                        )}
                    </div>
                </>
            )}
            ...
            <div className="mt-4 flex w-full overflow-auto gap-5">
                    ...
                    subs.map((s) => (
                        <div key={s.ID ?? s.Serial ?? `${s.CafeName}-${Math.random()}`} className="w-[450px] min-w-[450px] rounded-2xl bg-white border border-gray-100 p-4">
                            ...
                            <div className="flex items-center justify-center gap-4 mt-5">
                                {s.Status === 'active' ? (
                                    <>
                                        <button type="button" onClick={() => EnterSub(s.Serial)} className={`bg-teal-600 cursor-pointer text-white rounded-xl py-3 text-center font-bold text-sm hover:bg-teal-700 transition-colors ${s.Type === 'Owner' ? "w-1/2" : "w-full"}`}>ورود به پنل</button>
                                        {s.Type === 'Owner' && (
                                            <Link href={`/dashboard/subscription/${s.Serial}`} className="border border-teal-600 text-teal-600 rounded-xl py-3 w-1/2 text-center font-bold text-sm hover:bg-teal-50 transition-colors">مدیریت اشتراک</Link>
                                        )}
                                    </>

                                ) : s.Status === 'pending' ? (
                                    <>
                                        <Link href={`/invoice/${s.Invoice}`} className="bg-teal-600 text-white rounded-xl py-3 w-full text-center font-bold text-sm hover:bg-teal-700 transition-colors">مشاهده و پرداخت</Link>
                                    </>
                                ) : s.Status == "ready" && (
                                    <>
                                        <button
                ...
                {showUpdateModal && (
                    <UpdateModal serial={modalSerial} onClose={() => setShowUpdateModal(false)} />
                )}
            </div>
        </div>
    )
