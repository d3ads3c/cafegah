...
export default function Headbar() {
    ...
    return (
        <>
            <header className="w-full px-4 sm:px-8 md:px-20 lg:px-40 py-3 sm:py-4 fixed top-3 md:block z-10">
                <nav className="flex items-center justify-between backdrop-blur-xl p-3 sm:p-4 rounded-3xl relative shadow-xl md:shadow-none bg-white/50">
                    ...
                    <div className="flex items-center flex-shrink-0">
                        <Image src={"/img/logo/cafegah-transparent.png"} width={60} quality={100} height={60} alt="Cafegah Logo" className="max-w-[48px] sm:max-w-[60px]" />
                    </div>
                    {/* Desktop Menu */}
            ...
            {menuOpen && (
                <div onClick={() => setMenuOpen(!menuOpen)} className="fixed top-0 right-0 h-screen w-full md:hidden flex justify-between bg-black/30 backdrop-blur-lg p-4 z-40">
                    <div className="w-2/3 bg-white rounded-3xl h-full p-4">
                        <Image src={"/img/logo/FullLogo.png"} width={2000} height={2000} quality={100} className="max-w-[200px] mx-auto" alt="لوگو کافه گاه"></Image>
                        <div className="my-3 flex flex-col justify-between h-[90%]">
                            <ul className="flex flex-col gap-2 py-4 px-6 font-bold border-t border-gray-100">
                                <li><Link href="/" onClick={() => setMenuOpen(false)}>خانه</Link></li>
                                <li><Link href="/#features" onClick={() => setMenuOpen(false)}>قابلیت ها</Link></li>
                                <li><Link href="/#prices" onClick={() => setMenuOpen(false)}>قیمت ها</Link></li>
                                <li><Link href="/#faq" onClick={() => setMenuOpen(false)}>سوالات متداول</Link></li>
                                <li><Link href="/about-us" onClick={() => setMenuOpen(false)}>درباره ما</Link></li>
                            </ul>
                            <div className="flex flex-col gap-2 pb-4">
                                <Link href="/login" className="rounded-2xl py-3 px-4 border border-teal-600 text-teal-600 font-bold hover:bg-teal-600 hover:text-white duration-200 hover:shadow-xl hover:shadow-teal-600/30 text-sm text-center">پنل تامین کنندگان</Link>
                                <Link href="/login" className="rounded-2xl py-3 px-4 bg-teal-600 text-white font-bold shadow-xl shadow-teal-600/30 text-sm text-center">شروع کنید !</Link>
                            </div>
                        </div>
                    </div>
                    <div className="w-1/3">
